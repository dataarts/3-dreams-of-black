var UgcUI = function ( shared ) {
	...
	this.addListeners = function() {
		...
		var ui = document.getElementById('ugcui');
		var menus = ui.getElementsByClassName('menu');
		
		for (var i = 0; i < menus.length; i++) {
		
			menus[i].addEventListener('mouseover', function(e) {
			...
			menus[i].addEventListener('mouseout', function(e) {
				clearTimeout(this.hideTimeout);
				var buttons = this.getElementsByClassName('menu-buttons');
		...
		for (var i = 0; i < named.length; i++) {
			named[i].addEventListener('mouseover', function(e) {
				tooltip.style.display = 'inline-block';
				tooltip.innerHTML = this.getAttribute('title');
			}, false);
			named[i].addEventListener('mouseout', function(e) {
				tooltip.style.display = 'none';
			}, false);
		}
	...
	this.scale = function(s) {
		...
		div.firstChild.firstChild.setAttribute('transform', 'translate(10, 0) scale('+s+')');
	};
